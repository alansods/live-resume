# async-progress-states Specification
...
O percentual e a barra SHALL contar apenas as etapas **concluídas**: a etapa em andamento
SHALL NOT ser contada como concluída, e 100% SHALL aparecer somente quando a operação real
terminar. O timer alcança a última etapa nomeada muito antes de a chamada resolver — contar
a etapa corrente faria a barra anunciar 100% com a requisição ainda pendente, contradizendo
o próprio checklist ao lado, que ainda mostraria aquela etapa girando.
...
#### Scenario: O percentual não chega a 100% enquanto a chamada não termina
- **WHEN** o timer já alcançou a última etapa nomeada e a chamada de importação continua
  pendente
- **THEN** o percentual exibido é menor que 100%, e a última etapa aparece como em
  andamento em vez de concluída
...
sobrepostas e formatos, aplicar regras de leitura automática — cada uma com ícone de
estado e nada mais: SHALL NOT haver nota lateral de contagem. O checklist SHALL ser o
único indicador de espera da tela — não há bloco de skeleton
abaixo dele.

#### Scenario: Avançar já entra em estado de análise
- **WHEN** o usuário clica em "Avançar" na etapa 02
- **THEN** a etapa 03 é exibida imediatamente em estado de análise, e o realce do stepper
...
#### Scenario: O checklist de análise nomeia as cinco etapas
- **WHEN** a tela de análise está visível
- **THEN** as cinco etapas aparecem na ordem, cada uma com ícone de estado e sem nota
  lateral de contagem
...
progresso de exportação SHALL permanecer, na mesma coluna de largura máxima que o
formulário ocupava — não esticado à largura total da tela. O cartão SHALL exibir um
contador "N de M"
...
- **THEN** o formulário de seleção sai de tela e o cartão de progresso de exportação
  aparece sozinho, na mesma coluna de largura máxima que o formulário ocupava
...
tela SHALL exibir: um círculo de confirmação com ícone de check ao lado do título e do
parágrafo de agradecimento citando a quantidade de arquivos baixados; um bloco listando
cada arquivo gerado; um bloco de orientação de uso antes de enviar o currículo; e duas
ações lado a lado — baixar de novo (repete a geração) e começar um novo currículo (volta
à etapa 01, descartando a importação e as sugestões aceitas/dispensadas).
...
A exceção é a recusa do **arquivo enviado** na importação — formato não suportado, arquivo
grande demais, documento que não é currículo, cota esgotada. Ali o próximo passo não é tentar
de novo, é escolher outro arquivo, e a dropzone fica na tela: ver "Recusa do arquivo mantém a
dropzone na tela", em `app-shell-navigation`.
...
`aria-live="polite"` que anuncia a etapa nomeada corrente quando ela muda. Cada cartão
SHALL exibir um único indicador de carregamento animado — o ícone do item "em andamento"
do checklist —, e não um spinner adicional no cabeçalho duplicando a mesma animação.
Quando `prefers-reduced-motion: reduce` está ativo, esse indicador e a animação de
shimmer SHALL ser desligados, mas o texto da etapa corrente SHALL continuar visível como
...
#### Scenario: Um único indicador de carregamento por cartão
- **WHEN** um cartão de progresso está visível
- **THEN** o único elemento animado indicando carregamento é o ícone do item "em
  andamento" do checklist — o cabeçalho não repete essa animação num spinner à parte
